...
def optimize_image_bytes(image_bytes: bytes, max_dimension: int = 2000) -> bytes:
    """스마트폰 카메라 사진의 EXIF 회전 문제를 보정하고 고화질 초경량 WebP(2000px, 70%) 바이트를 반환"""
    try:
    ...
    with img_file as img:
        ...
        # Flatten anything with transparency or palette mode to RGB
...
async def create_submission(
    ...
    try:
        for idx, file in enumerate(files):
            ...
            # Everything is normalised to WebP, so the stored name always ends in .webp
            unique_filename = f"sub_{uuid.uuid4().hex[:10]}_p{idx + 1}.webp"
            optimized_bytes = optimize_image_bytes(content)

            if sb.is_supabase_enabled():
                url = sb.upload_image_to_storage(optimized_bytes, unique_filename, "image/webp")
            else:
...
def create_feedback(fb: FeedbackCreate):
    ...
    try:
        ...
        unique_filename = f"fb_{uuid.uuid4().hex[:10]}_p{fb.page_index + 1}.webp"
        annotated_image_url = ""
        
        if sb.is_supabase_enabled():
            annotated_image_url = sb.upload_image_to_storage(optimized_bytes, f"feedbacks/{unique_filename}", "image/webp")
        else:
